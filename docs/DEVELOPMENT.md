...
通常の `npm test` / CIからは分離し、必要時だけ実ブラウザで実行します。challengeが出た場合は突破せず失敗扱いにします。 Execution Handoffのlive検証目的でchallengeを意図的に発生させません。upstream `mcp-execution-handoff` dependencyはreview済みcommit archiveへimmutable pinし、`npm ci --ignore-scripts` で再現できる状態を維持します。Window WSS Handoff変更時はconsumer-local capture/input brokerを追加していないこと、WSS-only transport・exact-window target・Handoff input policyが意図した範囲を越えないこともstatic regressionで固定します。
