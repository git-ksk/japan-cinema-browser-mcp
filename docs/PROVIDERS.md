...
- 109 Phase 3 read-only seat adapterレビュー日: 2026-08-17
- Phase 4 checkout / seat-hold Discoveryレビュー日: 2026-08-17
...
| Provider | 公式root | 現在の自動化範囲 | Transaction boundary |
|---|---|---|---|
| TOHOシネマズ | `https://www.tohotheater.jp/` | reviewed public read surface / bounded read / 劇場・日付・作品・上映回 + read-only座席表 + TOHOのみrecommendation | Phase 4 Discovery済み。seat selection / checkout / purchaseはfalse。TOHO Gate 0待ち |
| イオンシネマ | `https://www.aeoncinema.com/` | reviewed public read surface / bounded read / 劇場・日付・作品・上映回 + read-only座席表 | server-side seat hold/release semantics未証明。seat selection / checkout / purchaseはfalse |
| 109シネマズ | `https://109cinemas.net/` | reviewed public read surface / bounded read / 劇場・日付・作品・上映回 + read-only座席表 | 10分seat holdは公式明記。seat mutation個別review前のためseat selection / checkout / purchaseはfalse |
...
- purchaser name / phone / email / birth dateは初期Phase 4でHuman-onlyとし、新しいPII ingress/logging/result pathを作らない
- providerがユーザー自身の同意を要求するlegal/terms consentはHuman-only
...
- seat selection / checkout等のsemantic mutationはautomatic replayしない
...
## Phase 4 Checkout Preparation / Human Handoff Discovery — 2026-08-17

詳細は [`PHASE4_CHECKOUT_DISCOVERY.md`](./PHASE4_CHECKOUT_DISCOVERY.md)、trackingは#48です。

Discoveryではseat clickを行わず、公開rendered UI・公式公開情報・Phase 3で証明済みのstateだけからstage/hold/Human boundaryを整理しました。

### TOHO

- guest purchase可能
- checkout stageはseat → ticket type → purchaser information → payment → review/completionの順で公式公開情報から確認
- FAQはdesired seat決定後15分以内の購入完了を要求
- exact hold triggerがseat activation / seat decision / 次stageのどこかは未証明
- conditional first vertical sliceを維持するが、#50 Gate 0通過前はseat clickしない

### AEON

- existing reviewed guest transitionでSmart Theater seat surfaceまで到達可能
...
- official purchase instructionsが10分seat holdを明記
...
- showtime / seats / ticket / amount等のmaterial factsはcurrent rendered UIから再取得
- ticket label/restrictionはprovider semanticを保持し、MCPはage/student/senior/accessibility/member eligibilityを推測しない
- exact intended seat setだけを一度操作し、alternate seat probing / speculative bulk hold / automatic retryをしない
- Human intervention後はfresh semantic action + provider/showtime/seat/hold context再検証
- purchaser PII / identity / consent / paymentはHuman-only
- `prepare_checkout` はfinal purchase/payment controlをsubmitしない
- `purchaseSubmission=false` はPhase 5まで維持

Implementation tracking:

- #49 provider-neutral contract/core
- #50 TOHO conditional first adapter + Gate 0
- #51 AEON provider review
- #52 109 provider review
...
- `予約購入` はread contextに含まれてもPhase 1 adapterからclickしない
...
公開schedule pageからmovie / start/end time / screen / format / 字幕・吹替 / explicit availabilityだけをcompact factへ正規化します。

`オンラインチケット購入` や上映回の購入導線はvisible read contextとして存在してもPhase 1 adapterからclickしません。
...
6. seat mutationの場合はhold trigger / release / expiry / stale semanticsをprovider別に証明
7. login/payment secretだけでなく、Phase 4初期scopeのpurchaser PIIもHuman入力のままであることを確認
8. CAPTCHA/MFA/3DS等で自動処理が停止することを確認
9. legal/terms consentをHumanへ戻すことを確認
10. semantic mutation / transactionがautomatic replayされないことを確認
11. checkout preparationとfinal submissionが分離されていることを確認
12. final purchaseではbrowser contextとmaterial transaction summaryをconfirmationへbinding
13. provider固有の制約を個別documentへ記録
...
TOHO / AEON / 109でread-only showtime + seatMapまでは揃いましたが、transaction capabilityのparityは目標にしません。

Phase 4はprovider-neutral contractを先に定義し、その後にTOHO Gate 0、AEON hold/release review、109 10-minute hold reviewを個別に通します。1 providerだけ安全性を証明できた場合は、そのproviderだけを昇格し、他providerの制約を弱めて合わせません。