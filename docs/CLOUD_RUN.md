...
Phase 4 TOHOのlocal Human Handoffはheadedなserver-owned Chrome + exact visible macOS windowを必要とします。Gate 0bのhistorical acceptanceはWebRTC、current Gate 1 acceptanceはWSS-only Window Handoffであり、Cloud Run headless profileの機能ではありません。`CINEMA_REMOTE_MODE=true` と `CINEMA_REMOTE_TAKEOVER=true` はそれぞれheadless/headedを要求し、意図的に同時有効化できません。
