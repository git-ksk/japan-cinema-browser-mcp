...
- TOHO Gate 0b Browser HandoffではCinemaがWebRTC/CDP takeover data planeを再実装せず、Handoff `BrowserHandoffAdapter`へexact managed Chrome PIDと `tap/scroll only` policyを渡す。text/keyはserver-side denyする。Humanがprovider自身のreview済み `terms_check` を明示的にONにした事実はDone後にread-onlyでboolean検証するが、Cinemaはそのcheckboxや `利用規約に同意して次へ` を自動操作しない。Done自体はseat hold/consent/purchase approvalとして扱わない
...
GitHub側ではsecret scanning / push protection / Dependabot security updates / Dependency Review / CodeQL advanced setup（security-and-quality suite）を有効化します。PRの通常CIはchanged pathsで選別し、runtime/buildに影響しないdocs-only変更ではNode test/buildとDependency Reviewを省略します。CodeQL analysisもMarkdown/textだけのPRは省略しますが、`codeql-gate` は常に実行します。analysis対象PRではgateがCodeQL job成功とPR-scoped alertsを確認し、Critical / High security alertまたはError quality alertをfail closedで拒否します。`main` pushとweekly scheduleではfull scanを維持します。`main` はrulesetでPR、required CI、`required-gate` / `codeql-gate`、linear history、force-push/delete禁止を強制します。
