...
- TOHO Gate 0b Browser HandoffではCinemaがWebRTC/CDP takeover data planeを再実装せず、Handoff `BrowserHandoffAdapter`へexact managed Chrome PIDと `tap/scroll only` policyを渡す。text/keyはserver-side denyし、Doneはseat hold/consent/purchase approvalとして扱わない
